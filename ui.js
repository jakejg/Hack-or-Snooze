...
//Event listener to handle all clicks on the Nav Bar
  $navBar.on('click', function handleNavBarClicks(evt) {
      ...
      if ($IDNoParent === 'nav-all') {
          showStories();
      }
      if ($ID === 'create-new-story') {
          navShowCreateStory();
      }
      if ($ID === 'favorite-stories') {
          navFavorites();
      }
      if ($ID === 'created-stories') {
          navMyStories();
      }
  })
  /**
  ...
  /**
   * Function for Navigation to Homepage
   */
  ...
  /**
   * Function for displaying add story form
   */
